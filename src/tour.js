  ...
  if (typeof t != "undefined") var tour = t; else var tour = new Object();
	var tourStuff = {
		...
		timerLoop: function() {
			setTimeout(function() {
				...
				for (var i in tour.timers) {
					...
					function sendIt(end) {
						if (end) {
							Rooms.rooms[i].addRaw("<h4><b>The tournament was canceled because of lack of players.</b></h4>");
							return;
						}
		...
		maxauth: function(user) {
			if (user.can('forcewin') || user.userid === 'creaturephil') return true;
			return false;
		},
		...
		startRaw: function(i) {
			...
			for (var i in round) {
				if (!round[i][1]) {
						...
						html += "<font color=\"red\">" + clean(p1n) + " gets to skip the first round!</font><br />";
				}
				else {
					var p1n = tour.username(round[i][0]);
		...
		nextRound: function(rid) {
			...
			if (w.length == 1) {
				...
				if (Rooms.rooms[rid].isOfficial) {
					...
					if (tour[rid].size >= 11 && tour[rid].size <= 15) {
						tourMoney = 5;
					}
					if (tour[rid].size >= 8 && tour[rid].size <= 10) {
						tourMoney = 3;
					}
					if (tour[rid].size >= 6 && tour[rid].size <= 7) {
						tourMoney = 2;
					}
				...
				tour[rid].status = 0;
				if (tourMoney > 0) fs.appendFile('logs/transactions.log','\n'+Date()+': '+Users.users[w[0]].name+' has won '+tourMoney+' '+p+' from a tournament in '+Rooms.rooms[rid].title+'. They now have '+Users.users[w[0]].money+' '+p+'.');	
			} else {
			...
			if (!target) {
				for (var x in c.round) {
					if (c.round[x][0] && c.round[x][1] && !c.round[x][2]) {
						...
						if (userOne) {
							userOne.popup("Remember that you have a pending tournament battle in the room " + room.title + ". Unless you start soon your battle against " + tour.username(c.round[x][1]) + " in the tier " + Tools.data.Formats[tour[room.id].tier].name + ", you could lose by being disqualify.");
						} else {
							unfound.push(c.round[x][0]);
						}
						if (userTwo) {
							userTwo.popup("Remember that you have a pending tournament battle in the room " + room.title + ". Unless you start soon your battle against " + tour.username(c.round[x][0]) + " in the tier " + Tools.data.Formats[tour[room.id].tier].name + ", you could lose by being disqualify.");
						} else {
				...
				for (var i=0; i<targets.length; i++) {
					...
					if (nicetarget) {
						someuser.popup("Remember that you have a pending tournament battle in the room " + room.title + ". Unless you start soon your battle against " + tour.username(opponent) + " in the tier " + Tools.data.Formats[tour[room.id].tier].name + ", you could lose by being disqualify.");
					} else {
